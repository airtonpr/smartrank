 ...
 *
 */
public class PPMFile implements xxxFile, Serializable {
    ...
    public PPMFile(InputStream stream) throws FileNotFoundException, IOException {
    ...
    private void readImage() throws FileNotFoundException, IOException, NumberFormatException {
        ...
        if (filename != null) {
            ...
            byte[] arr = InputStream2ByteArray.stream2Bytes(inputStream);
            isr = new ByteArrayInputStream(arr);
            length = arr.length;
        }
        ...
        if (id.charAt(0) == 'P') {
            ...
            if ((height * width + count * 2) < length) {
                weird = true;
            }
