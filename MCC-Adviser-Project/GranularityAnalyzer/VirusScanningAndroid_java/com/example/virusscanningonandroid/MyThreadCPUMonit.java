...
public class MyThreadCPUMonit implements Runnable
{
    ...
    public float getUsedMemorySize() throws IOException {
        ...
        while (true) {
            try {
                ...
                final float n4 = runtime.freeMemory();
                n = runtime.totalMemory();
                n2 = n - n4;
                n3 = n2 / n;
                Thread.sleep(2000L);
                this.manageFile.WriteFile(String.valueOf(String.valueOf(n) + ";" + n2 + ";" + n3));
                return n3;
            }
            catch (Exception ex) {
                ex.printStackTrace();
    ...
    @Override
    public void run() {
        this.manageFile = new ManageFile("Log-virus-local-cpu-" + System.currentTimeMillis() + ".txt");
        try {
            while (true) {
                this.manageFile.WriteFile(String.valueOf(100.0f * this.getUsedCPU()));
                try {
                    Thread.sleep(1000L);
                }
