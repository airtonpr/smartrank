...
public class ClassVisitor extends org.apache.bcel.classfile.EmptyVisitor {
	 ...
	 * Classes encountered. Its cardinality is used for calculating the CBO.
	 */
	private boolean notOffloadableClass;
	/**
	...
	public ClassVisitor(JavaClass jc, ClassMetricsContainer classMap) {
		...
		if (!ClassMetrics.isAndroidClass(myClassName) && !ClassMetrics.isJdkClass(myClassName)){
	...
	public void visitJavaClass(JavaClass jc) {
		...
		cm.setName(this.getMyClassName());
		cm.setVisited();
		if (jc.isPublic())
			cm.setPublic();
		ClassMetrics pm = null;
		if (!ClassMetrics.isAndroidClass(myClassName) && !ClassMetrics.isJdkClass(myClassName)){
			pm = cmap.getMetrics(super_name);
		}
	...
	public void registerCoupling(String className) {
		...
		boolean isNotTheCurrentClass = !myClassName.equals(className);
		boolean isNotAJKClass = MetricsFilter.isJdkIncluded() || !ClassMetrics.isJdkClass(className);
		if (isNotAJKClass && isNotTheCurrentClass) {
			efferentCoupledClasses.add(className);
		//	cmap.getMetrics(className).addAfferentCoupling(myClassName);
			if (isNotOffloadableClass(className)) {
				setNotOffloadableClass(true);
			}
		}
	}

	private boolean  isNotOffloadableClass(String className) {
		return (className.startsWith("android.hardware")||
				className.startsWith("android.media")||
				className.contains("BatteryManager")|| 
				className.contains("java.io.File")|| 
				className.contains("SharedPreferences")|| 
				 className.startsWith("android.location") );
	}
	...
	public boolean isNotOffloadableClass() {
		return notOffloadableClass;
	}

	public void setNotOffloadableClass(boolean notOffloadableClass) {
		this.notOffloadableClass = notOffloadableClass;
	}
	...
	public void end() {
		...
		cm.setIsNotOffloadable(isNotOffloadableClass());
		/*
